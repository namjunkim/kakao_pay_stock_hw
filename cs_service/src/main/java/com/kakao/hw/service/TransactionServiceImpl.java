...
public class TransactionServiceImpl implements TransactionService{
	...
	protected List<BusinessHistoryDTO> mappingToDtoTransactionHistory() {
		...
		try {
			transactionHistoryList = 
				readCSVService.readCSVFileFromLocal(resource.getURI().getPath(), 1);
		} catch(IOException e) {
			e.printStackTrace();
		}
	...
	protected List<BranchAccountInfo> mappingToDtoBrachAccountInfo() {
		...
		List<List<String>> branchAccountInfoLists = Lists.newArrayList();
		try {
			branchAccountInfoLists = 
				readCSVService.readCSVFileFromLocal(resource.getURI().getPath(), 1);
		} catch(IOException e) {
			e.printStackTrace();
		}
	...
	protected List<BranchInfoDTO> mappingToDtoBranchInfoDTO() {
		
		List<BranchInfoDTO> branchInfoDTOListResult = Lists.newArrayList();
		Resource resource = new ClassPathResource("branchInfo.csv");
		List<List<String>> branchInfoDTOLists = Lists.newArrayList();
		try {
			branchInfoDTOLists = readCSVService.readCSVFileFromLocal(resource.getURI().getPath(), 1);
		} catch (IOException e) {
	...
	protected BusinessHistoryDTO mappingBranchAccountInfoListIntoBusinessHistoryDTO(
...
//				&& branchAccountInfo.getAcctName().equals(businessHistoryDTO.getAcctName())
	...
	protected BusinessHistoryDTO mappingBranchInfoDTOListIntoBusinessHistoryDTO(
	...
	protected Map<String /*year*/, List<BusinessHistoryDTO>> divideBusinessHistoryDTOListByYear (List<BusinessHistoryDTO> businessHistoryDTOList) {
	...
	protected Map<String /*year*/, Map<String/*customer*/, Long /*sumOfAmount*/>> 
...
//			customerSumByYearMap.put(businessHistoryByYear.getKey(), null);
	...
	// 아예 없는분
	protected Map<String/*year*/, Map<String /*name*/, BranchAccountInfo>> noneTransction(Map<String /*year*/, List<BusinessHistoryDTO>> businessHistoryByYearMap, List<BranchAccountInfo> branchAccountInfoList) {
	...
	// 취소분
	protected Map<String/*year*/, Map<String /*name*/, List<BusinessHistoryDTO>>> getOnlyCancellation (Map<String /*year*/, List<BusinessHistoryDTO>> businessHistoryByYearMap) {
	...
	protected Map<String /*brName*/, List<BusinessHistoryDTO>> divideBusinessHistoryDTOListByBrName (List<BusinessHistoryDTO> businessHistoryDTOList) {
	...
	protected List<AmountSumByBranchByYear> getAmountSumByBranchByYear (Map<String /*year*/, List<BusinessHistoryDTO>> businessHistoryDTOByYear) {
